...
bestActionSpec bp@(BanditProblem {bpIsDeterministic = isDet})
 ...
 = let subSpecs = map (bestActionSpec bp 0 . bbtNode) subtrees
       ...
       ((bestSon, _) , _) = maximalAndRestBy
                              (measure . snd)
                              $ map (\s -> (s, bestNode bp s)) sons
...
findBest budget beta problem playBudgetM =
    do liftIO $ do --hSetBuffering stdout NoBuffering
       ...
       let tree = last [c | (a,b,c) <- initRes : allResults]
       liftIO $ print tree
       let actSpec = bestActionSpec problem 0 $ bbtNode tree
       liftIO $ print (actSpec, bestNode problem $ bbtNode tree)
       return actSpec
