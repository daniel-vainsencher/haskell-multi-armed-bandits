...
-}
initTree :: Monad m => BanditProblem m a -> m (Maybe [a], Float, BanditTree a)
...
interactWithProblem :: Monad m => BanditProblem m a -> [a] -> ActionNovelty -> m (Decision a)
...
bibPlayAction :: Monad m => Int -> [Float] -> m (Float, [[Float]])
bibPlayAction n m
  = do return $ case m of
